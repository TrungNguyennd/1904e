...
!empty($_POST) dùng để kiểm tra biến có tồn tại hay không*/
if(isset($_POST)&& !empty($_POST)){
    ...
    if(!isset($_POST["name"]) || empty($_POST["name"]) ){
        $errors="tên nhân viên không họp lệ";
    }
    if(!isset($_POST["address"]) || empty($_POST["address"]) ){
        $errors="địa chỉ  nhân viên không họp lệ";
    }
    if(!isset($_POST["salary"]) || empty($_POST["salary"]) ){
        $errors="lương nhân viên không họp lệ";
    }
    //$errors rỗng tức là có lỗi
    if(empty($errors)){
        $name=$_POST["name"];
